<?php
// filepath: c:\Users\zaour\source\web1a\project\project-4-test\socks2\resources\views\pages\admin\products\[Product]\edit.blade.php
...
// Get product from URL parameter correctly

try {
    // With Laravel Folio, the parameter name matches the directory name [Product]
    $product = Product::findOrFail($id);
} catch (\Exception $e) {
    // If product not found, redirect to products page with an error
    return redirect()->route('admin.products')
        ->with('error', 'Product not found: ' . $e->getMessage());
}
...
if (request()->isMethod('post')) {
    ...
    if (request()->hasFile('image')) {
        ...
        $validated['afbeelding'] = Storage::url($imagePath);
    }
    ...
    $product->update([
        ...
        'afbeelding' => $validated['afbeelding'] ?? $product->afbeelding,
    ]);
...
<x-layouts.admin title="Edit Product">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold">Edit Product: {{ $product->naam }}</h1>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                required
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-zinc-900 dark:text-white"
                            >{{ old('omschrijving', $product->omschrijving) }}</textarea>
                            @error('omschrijving')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                <div 
                                    x-data="{ 
                                        imagePreview: '{{ $product->afbeelding }}',
                                    ...
                                    x-init="$el.querySelectorAll('input[type=file]').forEach(el => el.addEventListener('change', function() { updatePreview(this); }))"
                                ...
                                >
                                    ...
                                    <img x-bind:src="imagePreview" class="h-full w-full object-cover" />
                                </div>
                            </div>
                            @error('image')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
